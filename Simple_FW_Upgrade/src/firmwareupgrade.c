...
void FirmwareUpgrade(void)
{
    ...
    {   // Regular code page needs to be programmed to code image area
        desAddress = (page - FIRMWARE_UPGRADE_CODE_START_PAGE) * CODE_PAGE_SIZE + CODE_IMAGE_START_ADDRESS;
    ...
    {   // First page received. We need start SIB firmware upgrade
        ...
        // Erase code image
        err = fpEraseCodeFlash(CODE_IMAGE_START_ADDRESS,TOTAL_CODE_FLASH_BLOCK);
        if(err == FSP_SUCCESS)
        {   // Send "Good"
...
//            eepSaveBootInfor(&bootInfor); // We are not saving the boot info in EEPROM. TO DO MVE
            // Need make sure data flash is completed!!!
